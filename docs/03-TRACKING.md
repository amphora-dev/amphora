...
> 最后更新: 2026-07-13 · 资产获取轨: imagefs.tzst 真资产到手 (SHA 锁) + 真机 (Adreno arm64) rootfs 提取验证通过; **preparer 真机验证通过** (Proton/Box64 .wcp 本地安装 + 容器创建 + extractGraphicsDriverFiles + envVars/wrapper.tzst 提取全验)
...
- ⏭ 下一步: `:core:content` `BundledContentSource` (用 manifest SHA, 替代测试的宿主 curl+adb push 临时路径); P3 `:app` GameSessionScreen. **preparer 真机验已完成** (见下 §P2 #7).
