...
- 📦 **最终包结构定稿** (2026-07-30): 能缩尽缩——自建 imagefs 唯一默认 rootfs；废止 `extra_libs`→strip 后 `mesa-gl`；砍 vkBasalt/BCn/包内 Turnip/`layers` 默认；字体共享单文件 + 瘦 pattern；ddraw/Turnip/FFmpeg/validation 可选；wincomponents 保持 tzst。默认资产面 ~230–240 MB vs 现状 ≳450 MB。详见 §0.6。
