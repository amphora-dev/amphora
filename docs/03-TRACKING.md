...
- ✅ **adrenotools hooks 收敛为单份** (2026-08-05): guest / host 都用 `wrapper.tzst` → `imagefs/usr/lib`（CI pin `8483dfd`）。APK 排除 4 个 hook `.so`；host `vulkan.c` 改读 `ImageFs.getLibDir()`（`AppUtils` 已删，旧 `nativeLibraryDir` 路径早已失效）。
- 🗑 **删除空壳 `:core:ui`** (2026-08-05): 从未 `include`，只留 `build.gradle.kts`；无源码。
- 🔧 **adrenotools 路径演进纪要** (2026-07-29→08-05): 曾试 APK `nativeLibraryDir` 作 guest HOOKS_PATH → guest 报 `Couldn't preload the hook implementation`；改回 imagefs。`driver.h` 的 MUST=`nativeLibraryDir` 实为打包语境；`hookLibDir` 只是 linker namespace 的 `ld_library_path`。guest `libadrenotools.so` 与 hooks 必须同 vintage——现由自建 `wrapper.tzst` 同包提供。
- 📦 **目标包结构拍板** (2026-07-30, hooks 条目 2026-08-05 修正): **`wrapper`（含 hooks）独立发版**——不焊进 imagefs rootfs；hooks **只认** `imagefs/usr/lib`（随 wrapper 解出）；APK 不再带第二份。详见 [`04-ASSET-MANIFEST.md`](04-ASSET-MANIFEST.md) §0.6。
- 📦 **最终包结构定稿** (2026-07-30): 能缩尽缩——自建 imagefs 唯一默认 rootfs；废止 `extra_libs`，其中 strip 后的 Mesa `libGL`+`libglapi` **并入 imagefs**；砍 vkBasalt/BCn/包内 Turnip/`layers` 默认；字体共享单文件 + 瘦 pattern；ddraw/Turnip/FFmpeg/validation 可选；wincomponents 保持 tzst。默认资产面 ~230–240 MB vs 现状 ≳450 MB。详见 §0.6。
...
- ⏭ 下一步: **Exit ANR 根治**；v0.2 候选 settings / 键盘手柄 / Pulse·FEX 扩展。详见 [`05-ARCHITECTURE.md`](05-ARCHITECTURE.md) §9。
