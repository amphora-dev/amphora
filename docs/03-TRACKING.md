...
> 最后更新: 2026-07-14 · 资产获取轨: imagefs.tzst 真资产到手 (SHA 锁) + 真机 (Adreno arm64) rootfs 提取验证通过; **preparer 真机验证通过** (Proton/Box64 .wcp 本地安装 + 容器创建 + extractGraphicsDriverFiles + envVars/wrapper.tzst 提取全验); **`:core:content` `BundledContentSource` 落地** (manifest JSON + SHA-256 校验 + ARCHIVE/WCP 双路径, `EngineModule` 绑定, 替代 curl+push 生产路径 -- 详见 §P2 #8); **`.wcp` SHA 锁 ✅ (gap #1, 2026-07-14)**; **`BundledContentSourceTest` 真机验 ✅ (3/3, §P2 #10)**; **P3 `GameSessionScreen` + launch 链落地** (compile-only, `2a2078a`+`0404922`, 详见 §P3)
...
- ⏭ 下一步: P4 收尾 -- `:core:container` `ContainerManager` 真实现 (解锁 P3 launch 链端到端) + launcher exe picker + 分辨率选择 -> 构 `LaunchSpec` + RFC §8 验收 (启动 .exe, 画面+触屏+音频)。**P3 `GameSessionScreen` + launch 链已落地 (compile-only)** -- `WineEngineImpl.launch` 真接线 (XSDA setupXEnvironment 移植, Steam/shortcut/recording/arm64ec/WinHandler 剥), `XServerSessionHandle` (pause/resume/stop), 真 sinks (XServer/ALSAClient), `GameSessionScreen` (AndroidView{XServerSurfaceView} + 触屏 overlay + 生命周期), 详见 §P3。launch 链在 P4 `ContainerManager` stub 处抛 (compile-only), 端到端验待 P4。`EngineModule.provideContainerManager` stub 保留至 P4 (同 DIP, concretion 落 :core:engine 时删)。
