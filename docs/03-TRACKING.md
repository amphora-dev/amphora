...
> 最后更新: 2026-07-14 · 资产获取轨: imagefs.tzst 真资产到手 (SHA 锁) + 真机 (Adreno arm64) rootfs 提取验证通过; **preparer 真机验证通过** (Proton/Box64 .wcp 本地安装 + 容器创建 + extractGraphicsDriverFiles + envVars/wrapper.tzst 提取全验); **`:core:content` `BundledContentSource` 落地** (manifest JSON + SHA-256 校验 + ARCHIVE/WCP 双路径, `EngineModule` 绑定, 替代 curl+push 生产路径 -- 详见 §P2 #8); **`.wcp` SHA 锁 ✅ (gap #1, 2026-07-14)**; **`BundledContentSourceTest` 真机验 ✅ (3/3, §P2 #10)**
...
- ⏭ 下一步: P3 `:app` GameSessionScreen (D9). **P2 资产获取轨全闭环** -- `BundledContentSource` 落地 (§P2 #8), preparer 真机验已完成 (§P2 #7). ~~`.wcp` SHA 锁~~ ✅ (gap #1, 2026-07-14 -- wine/box64 SHAs 已入 manifest)。build 时资产 staging Gradle 任务 ✅ 落地 (`:app:stageBundledContent`, §P2 #9)。`BundledContentSourceTest` 真机验 ✅ (§P2 #10, 3/3)。
