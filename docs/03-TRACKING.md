...
- 🔧 **adrenotools hook 路径已修** (2026-07-29): `ADRENOTOOLS_HOOKS_PATH` 原先指 `imageFs.getLibDir()`，但 `adrenotools/driver.h:22` 明确要求该路径**必须**是 `getApplicationInfo().nativeLibraryDir`——否则 `adrenotools_open_libvulkan` 照样返回有效 handle，但 hook 失败后驱动**静默回退系统 Adreno**，或 `vkEnumeratePhysicalDevices` **返回 0 设备**。已改为 `nativeLibraryDir`（APK 内已带我们自建 submodule `8483dfd` 编的 4 个 hook，`useLegacyPackaging=true` 保证解压落盘）。这可能与「盲注 `ADRENOTOOLS_*` 导致 DX9–11 挂」的旧现象同源，待真机复验。
  - 🔍 **约束强度已核实 + 发现 ABI 风险** (2026-07-30): 读 `src/driver.cpp:75` —— `hookLibDir` 只是 `android_create_namespace` 的 **`ld_library_path`**，平台**不**要求等于 `nativeLibraryDir`；`driver.h` 那句 MUST 的语境是打包（`useLegacyPackaging=false` 时该目录为空）。所以旧做法（hooks 在 `wrapper.tzst` → `imagefs/usr/lib`，HOOKS_PATH 指 `imageFs.getLibDir()`）**技术上可行**（app 私有 filesDir 可 dlopen + targetSdk 28 legacy domain），改指 `nativeLibraryDir` 的真实收益是消除三份 hook 版本漂移。**顺带查出一条 ABI 边界并已实测**：`libadrenotools` 经 `initHookParam(new HookImplParams(...))` 跨 `.so` 传含 5 个 `std::string` 的 C++ 结构体，故它与 4 hook 必须同版本。guest `libvulkan_wrapper.so` 是上游 blob（NEEDED 上游 `libadrenotools.so`），hooks 来自自建 `8483dfd` = 混搭。**比对结果一致**：上游 blob（NDK r28c，stripped）仍导出 weak 构造签名 `_ZN14HookImplParamsC2EiPKcS1_S1_S1_S1_P23adrenotools_gpu_mapping`，逐参对齐 submodule 源码（`int` + 5×`const char*` + `gpu_mapping*`），字段集同 → 布局同（NDK libc++ `std::string` 恒 24B）。**改动安全**。另测出 guest wrapper 对 libadrenotools 的未定义符号只有 `adrenotools_open_libvulkan` 一个，故备选方案「自建 `libadrenotools` 覆盖 wrapper 内那份」符号面可行但当前无需执行。规则保留：任一侧升级须重新比对该 mangled 签名。详见 [`04-ASSET-MANIFEST.md`](04-ASSET-MANIFEST.md) §0.6。
  - 🎯 **hooks 收敛到一份还差 2 步** (2026-07-30): **运行时引用已唯一**（APK `nativeLibraryDir`；实测 `app-debug.apk` 内 4 个 hook 齐全，submodule `8483dfd` 编出，`libadrenotools` 静态链进 `libwinlator.so` 不单独出 `.so`）。但**磁盘仍会落第二份**，因为 (a) 官方 `imagefs.tzst` 自带一份，(b) `extractGraphicsDriverFilesCore` firstTimeBoot + `installAdrenotoolsDriverIfNeeded` 仍解官方 `wrapper.tzst` 到 `imagefs/usr/lib/`。消除：**切自建 imagefs**（配方不打 hooks）+ **重打 `wrapper.tzst` 剔除 4 个 hook**（只留 `libvulkan_wrapper.so`+`libadrenotools.so`+ICD json）。残留文件无人引用故无害，但会干扰版本比对。注意 `winlator-imagefs/build-native-libs.sh` 也编 hooks 进 `native-libs.tar.xz`——那是该仓对比上游 blob 的复现验证产物，不进 `imagefs.txz`、不是 amphora 来源，勿接成第四份。
- 📦 **目标包结构拍板** (2026-07-30): **`wrapper` + adrenotools hooks 单独更新**——不焊进 imagefs；hooks 只认 `nativeLibraryDir`；`wrapper.tzst` 独立 ARCHIVE/SHA；与 hooks 可同一次 Release 共版，但绝不是 imagefs 附件、也不并回 `extra_libs`。详见 [`04-ASSET-MANIFEST.md`](04-ASSET-MANIFEST.md) §0.6。
- 📦 **最终包结构定稿** (2026-07-30): 能缩尽缩——自建 imagefs 唯一默认 rootfs；废止 `extra_libs`，其中 strip 后的 Mesa `libGL`+`libglapi` **并入 imagefs**（判据不是「有依赖」——`wrapper` 同样依赖 imagefs 的 `libxcb`/`libdrm`/`libandroid-sysvshm` 却独立发版；而是 libGL **无插拔需求**（可换的是下层 Vulkan 驱动，非 libGL）+ **拆了不省体积**（私有件只有 1.4 MB 的 `libglapi`，其余 `libdrm`/`libX11`/`libz`/`libzstd`/`libc++_shared` 是 wrapper/Turnip/Wine 共用的底座，必须留 imagefs）。wrapper/hooks 相反：hooks 物理上必须在 `nativeLibraryDir`，Vulkan 驱动是真插拔点）；砍 vkBasalt/BCn/包内 Turnip/`layers` 默认；字体共享单文件 + 瘦 pattern；ddraw/Turnip/FFmpeg/validation 可选；wincomponents 保持 tzst。默认资产面 ~230–240 MB vs 现状 ≳450 MB。详见 §0.6。
- 🩹 **Wine 全白窗口 = 没字体，根因是 patchelf 破坏 ELF 段对齐** (2026-08-01): 现象是窗口画得出来但一个字都没有。日志指向字体而非图形：`Error initializing native libfreetype.so (… cannot find "libpng16.so.16" from verneed[0] …)` + `Wine cannot find the FreeType font library`。把 libpng 强行先加载，报错变成 `cannot find "_chunk_fn" from verneed[0]`——`_chunk_fn` 是符号名不是库名，说明运行期字符串表整个错位。根因在 imagefs `packages/compress/libpng.sh` 的 `patchelf --set-soname`：它把改写后的 `.dynstr` 放进新追加的 LOAD 段，而该段 `p_vaddr` 与 `p_offset` 不再页同余，加载器按 `DT_STRTAB` 读出的 verneed/verdef 名字全是乱码 → libpng 加载失败 → DT_NEEDED 它的 libfreetype 一起失败 → Wine 无字体。**`readelf` 走 section header 看一切正常**，所以 CI 一直没拦住；只有真机 `dlopen` 会炸。修法：SONAME 在链接时给全（`-Wl,-soname,libpng16.so.16`），不再事后 patch；`ci/verify/wine-deps.sh` 增加全量 `.so` 的 LOAD 段同余断言（扫 v29：134 个库只有 libpng 中招）。免踩同类坑的通用做法见 imagefs `docs/analysis/ELF-PITFALLS.md`；配方风格参照 [`KreitinnSoftware/MiceWine-Packages`](https://github.com/KreitinnSoftware/MiceWine-Packages)（它的 libpng 是朴素 autotools、不 patchelf，所以上游没这问题）。**排查提速**：不必启动 Wine，`run-as app.amphora` 直接跑 imagefs 里的 `usr/bin/fc-cache --version` 就能拉起 fontconfig→freetype→libpng→zlib 整条链。
- ✅ **`extra_libs.tzst` 已废止 / Mesa GL 自建并入 imagefs** (2026-08-01): imagefs 新增包 `packages/graphics/mesa-gl.sh`——上游 Mesa release 源码交叉编译出 `libGL.so.1`（`gallium-drivers=zink,softpipe` + `glx=xlib`；xlib GLX 强制要一个软件光栅化器，但 `sw_helper` 先探 zink，且 launch 显式下发 `GALLIUM_DRIVER=zink`），链接画像与 wrapper ICD 同源（meson `system=linux` + `-D__TERMUX__` + 共用 patch 0003），故 `DT_NEEDED` 不含 `liblog`/`libcutils`/`libsync`。两处 Mesa 分建是刻意的：Vulkan 驱动是真插拔点（独立 `wrapper.tzst`），libGL 没有插拔需求（随 rootfs）。`-Dandroid-stub` 自 Mesa 25.3 起要求 `platforms=android`，与 Termux 画像互斥，改用 `vendor/mesa-gl-patches/0001` 让 `vk_android_native_buffer.h` 在 `__TERMUX__` 下取通用 `buffer_handle_t`。Amphora 侧删掉 firstTimeBoot 对 `graphics_driver/extra_libs.tzst` 的提取，manifest 删该 `runtimeAssets` 条目（-21.1 MB 下载面，imagefs 约 +3 MB）。
- 📦 **资产压缩包已自有化** (2026-07-29): 新建镜像仓 [`atowerlight/amphora-assets`](https://cnb.cool/atowerlight/amphora-assets)，`assets.json` 固定 24 个资产（497.5MB）的 SHA-256 + size（从本 manifest 派生），CI 校验后覆盖发布固定标签 `assets`。**前置阻塞**：该仓目前是 **Private**，CNB OpenAPI 改不了可见性（`PATCH /{repo}` 只收 description/license/site/topics，传 visibility 静默忽略仍返回 200），须在 Web 界面改成公开，匿名 `curl` 到 Release 附件返回 200 后再把本 manifest 的 `remoteUrl` 切过去。三个无 `remoteUrl` 的 wcp（box64/DXVK/VKD3D）的 catalog 解析结果已固化进 `assets.json`。
- 🧩 **imagefs 自建缺口已定量** (2026-07-29): 自建 [`winlator-imagefs`](https://cnb.cool/atowerlight/winlator-imagefs) 产物 18MB vs 官方 199.8MB / 10892 条目 / 667 个 `.so`。差异靠 Proton wcp 里 Wine unix 侧 31 个 `.so` 的 `NEEDED`（22 项）+ `dlopen` 字符串反推，真实缺口只 4 组：**gnutls**（`bcrypt`/`secur32` dlopen，缺则无 TLS——影响面最大）、**FFmpeg**（`winedmo.so`）、**GStreamer**（`winegstreamer.so`）、adrenotools 4 个 hook（已由上一条改到 APK 解决）。官方那 191MB 的绝大部分是 Wine 用不到的 userland（`terminfo` 2942 条目 / `doc` 1951 / `locale` 1077 / `man` 612 / Tcl 858 / `aomenc`/`x264`/`ffmpeg` 可执行文件）。
  - ⚠️ **FFmpeg soname 错配（官方 imagefs 也中）**: `winedmo.so` NEEDED 写死 `libavcodec.so.62`/`libavutil.so.60`/`libavformat.so.62`（FFmpeg 8.0），官方 imagefs 装的是 **7.1**（61/59/61）→ **即使用完整官方镜像 `winedmo` 也加载不了**。自建侧已补 FFmpeg 8.0，并由 `ci/verify-wine-deps.sh` 在构建时断言硬依赖 + 打印实测 SONAME，防止升级大版本时再静默降级。
...
| compileSdk / targetSdk / minSdk / NDK | 37 / **28** / 26 / r28 (28.2.13676358) - targetSdk 28 因 `filesDir` 执行 box64/Wine (SELinux) |
