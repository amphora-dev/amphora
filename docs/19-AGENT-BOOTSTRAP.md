...
- Soft IME commit → 同 `KEYBOARD_EVENT` 管（`WineInputConnection` + `WineAndroidImeCommit`）；CJK → `KEYEVENTF_UNICODE`（`nativeSendUnicodeChar`）；composition 仍本地
...
键盘（硬件 KEYCODE / `adb input keyevent`）：`key hwnd=… ok=true`；native `keyboard hwnd=… vkey=…`。`adb shell input keyevent 29`（A）或 `66`（ENTER）；`adb shell input text hello` 走 KEYCODE 注入。Soft IME：触摸后应弹出键盘；commit ASCII 应见同样 `key hwnd=…`；CJK commit 应见 `IME unicode` + native `keyboard unicode uchar=`。**仍开**：CJK composition 本地；真机 CJK soft IME 眼验（`adb input text` 仅 ASCII）。
...
`KEYEVENTF_UNICODE`；DPI 96；hostScale 多分辨率单测（`034b38e`）；游戏轨 AHB
已合入 `proton_11.0` @ `0a64ebc`，WCP 已发，HA262 Present **v9c PASS**（公开 pin）。
