...
- Soft IME commit → 同 `KEYBOARD_EVENT` 管（`WineInputConnection` + `WineAndroidImeCommit`）；CJK → `KEYEVENTF_UNICODE`（`nativeSendUnicodeChar`）；composition 仍本地（UI 下一拍）
...
**已落地**：MOTION（`d3a7bd5`）+ 硬件 KEYBOARD（`35c9921`）+ soft IME **commit**（`WineInputConnection` → KEYBOARD_EVENT）+ CJK **unicode**（`KEYEVENTF_UNICODE` / `nativeSendUnicodeChar`）。Composition 仍本地。
...
键盘（硬件 KEYCODE / `adb input keyevent`）：`key hwnd=… ok=true`；native `keyboard hwnd=… vkey=…`。`adb shell input keyevent 29`（A）或 `66`（ENTER）；`adb shell input text hello` 走 KEYCODE 注入。Soft IME：触摸后应弹出键盘；commit ASCII 应见同样 `key hwnd=…`；CJK commit 应见 `IME unicode` + native `keyboard unicode uchar=`。**Debug unicode 自动冒烟 PASS**（`702b165`，冷启 `IME_UNICODE_TEXT`；ART `ha262-ime-unicode-auto-20260916-140749`）。**仍开**：host composing overlay；真机 CJK soft IME 眼验（`adb input text` 仅 ASCII）。
...
`KEYEVENTF_UNICODE`（**HA262 debug unicode 自动冒烟 PASS** @ `702b165`）；DPI 96；
hostScale 多分辨率单测（`034b38e`）+ HA262 旋转已验；guest 分辨率预设目录；
游戏轨 AHB 已合入 `proton_11.0` @ `0a64ebc`，WCP 已发，HA262 Present **v9c PASS**。

**本拍优先：**

1. docs/16：第二台真机 / 分屏仍开；guest 分辨率**设置页 UI** 仍开。
2. **host-local composing overlay**（`ImeUiState` chip；勿发明 IMM32/TSF）；可选真机
   CJK soft IME 眼验。
