...
> 相关：[`03-ASSET-MANIFEST.md`](03-ASSET-MANIFEST.md)（AIO 2.1.0）、[`04-WINEANDROID-DISPLAY.md`](04-WINEANDROID-DISPLAY.md)、[`05-AHB-IMPORT-PRESENT.md`](05-AHB-IMPORT-PRESENT.md)。  
...
| 官方预编译 AIO Graphics Test **2.1.0** | `core/content/.../winnative/Graphics-Test-{32,64}bit.exe`；见 docs/03 |
...
- 当前假报与 remap 在 **win32u 全局**，**没有** `amphora_wsi_wanted()` 门闩。这与 docs/04「不要在 `win32u/vulkan.c` 加 android 分支」的意图冲突：功能上不是 `#ifdef android`，但是把 Android Adreno 缺模式问题做成了全驱动行为。
...
3. **层修复方向（尚未改代码）：** 若仍要假报 PresentModes，应闸在 Amphora / `amphora_wsi_wanted()`（或等价 Android 条件），而不是长期留在全局 win32u；与 docs/04 对齐。
