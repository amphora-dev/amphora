...
- **2026-09-26 · 删除 X11/Java 会话链**：`SessionActivity` / `GameSession*` / `HostPerformance*` / `Touchpad*` / `RtsGestureController` / `WineAndroidLaunchGate` / `WineEngine(Impl)` / `XServerSessionHandle` / `XServerSinks` / `StubInputSink` / `GameSessionSurface` / `DisplayBackend` / `EXTRA_DEBUG_X11` 已删（59 文件、-7335 行），`SessionLaunch` 只开 `WineAndroidSessionActivity`；Wine 命令拼装移到 `WineLaunchCommands.kt`。保留仍被 wineandroid 使用的 `XEnvironment`、`GuestProgramLauncherComponent`、`XServerWineSessionPreparer`、`WineHeadlessRunner` 与 `core/engine/src/main/java/**`（Winlator Java 与 native JNI 拆除是下一刀）。
