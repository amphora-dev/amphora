...
- **2026-09-26 · 删除 X11/Java 会话链**：`SessionActivity` / `GameSession*` / `HostPerformance*` / `Touchpad*` / `RtsGestureController` / `WineAndroidLaunchGate` / `WineEngine(Impl)` / `XServerSessionHandle` / `XServerSinks` / `StubInputSink` / `GameSessionSurface` / `DisplayBackend` / `EXTRA_DEBUG_X11` 已删（59 文件、-7335 行），`SessionLaunch` 只开 `WineAndroidSessionActivity`；Wine 命令拼装移到 `WineLaunchCommands.kt`。保留仍被 wineandroid 使用的 `XEnvironment`、`GuestProgramLauncherComponent`、`XServerWineSessionPreparer`、`WineHeadlessRunner` 与 `core/engine/src/main/java/**`。删前状态留 tag `x11-reference`；wineandroid 尚缺的功能（音频服务、触控模式、会话抽屉、HUD 等）见 [`04`](04-WINEANDROID-DISPLAY.md) §7.1 对齐表。**下一步先补音频服务**（删除前 wineandroid 就没起 Pulse/ALSA），Winlator Java / native JNI 按对齐表逐项拆，未对齐的组件不删。
