...
| Guest 图形 | Turnip wrapper ICD + DXVK + VKD3D；OpenGL→EGL/Zink，DirectDraw→DxWrapper Dd7to9 或 cnc-ddraw→D3D9/DXVK（不回退 WineD3D） | host renderer 与 guest 共用 adrenotools-wrapped driver |
