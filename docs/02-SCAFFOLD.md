...
| compileSdk / targetSdk / minSdk | **37** / **36** / 30 | 平台包名 `platforms;android-37.0`（带 `.0`）。发布的 Box64、Vulkan wrapper 与 wineserver 依赖 API 30 的 `LIBC_R`；`filesDir` ELF 由 `/system/bin/linker64` 首启，`libamphora-exec.so` 拦截后续 `exec*` |
