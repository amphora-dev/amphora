...
#   name|cwd|task.md|max-time|RESULT_KEY[|tier]   (tier: fast|code|deep, default fast)
set -euo pipefail
...
steps=0
while IFS='|' read -r name cwd task max_time key tier || [ -n "$name" ]; do
...
run_chain() {
  ...
  while IFS='|' read -r name cwd task max_time key tier <&3 || [ -n "$name" ]; do
    case "$name" in ''|'#'*) continue ;; esac
    local run_dir
    if run_dir="$(cd "$base" && "$here/dispatch.sh" --orchestrator "$orchestrator" \
        --name "$name" --cwd "$cwd" --task "$task" \
        --max-time "${max_time:-40m}" --result-key "$key" --tier "${tier:-fast}" | head -1)" &&
