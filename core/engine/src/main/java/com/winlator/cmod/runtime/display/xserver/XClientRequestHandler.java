...
import com.winlator.cmod.runtime.display.xserver.requests.AtomRequests;
import com.winlator.cmod.runtime.display.xserver.requests.CursorRequests;
import com.winlator.cmod.runtime.display.xserver.requests.DrawRequests;
import com.winlator.cmod.runtime.display.xserver.requests.ExtensionRequests;
import com.winlator.cmod.runtime.display.xserver.requests.FontRequests;
import com.winlator.cmod.runtime.display.xserver.requests.GrabRequests;
import com.winlator.cmod.runtime.display.xserver.requests.GraphicsContextRequests;
import com.winlator.cmod.runtime.display.xserver.requests.KeyboardRequests;
import com.winlator.cmod.runtime.display.xserver.requests.PixmapRequests;
import com.winlator.cmod.runtime.display.xserver.requests.SelectionRequests;
import com.winlator.cmod.runtime.display.xserver.requests.WindowRequests;
...
public class XClientRequestHandler implements RequestHandler {
  ...
  private void sendServerInformation(XClient client, XOutputStream outputStream)
      throws IOException {
    ...
    short additionalDataLength =
        (short)
            (8
                + (2 * pixmapFormatCount)
                + ((vendorNameLength + 3) / 4)
                + ((40 + 8 * client.xServer.pixmapManager.supportedVisuals.length + 24) + 3) / 4);
    ...
    try (XStreamLock lock = outputStream.lock()) {
      ...
      outputStream.writeInt(client.xServer.defaultColormapId);
  ...
  private boolean handleAuthRequest(
    ...
    if (majorVersion != 11)
      throw new UnsupportedOperationException(
          "Unsupported major X protocol version " + majorVersion + ".");
  ...
  private boolean handleNormalRequest(
      XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException {
    ...
    try {
      switch (opcode) {
        ...
        case ClientOpcodes.CHANGE_WINDOW_ATTRIBUTES:
          try (XLock lock =
              client.xServer.lock(
                  XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.CURSOR_MANAGER)) {
        ...
        case ClientOpcodes.MAP_WINDOW:
          try (XLock lock =
              client.xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.INPUT_DEVICE)) {
        ...
        case 36: // X_GrabServer
          try (XLock lock = client.xServer.lockAll()) {
            ...
            Log.d(
                ...
                "X_GrabServer request handled successfully:" + outputStream.buffer.position());
          }
          break;
        ...
        case 37: // X_UngrabServer
          try (XLock lock = client.xServer.lockAll()) {
            ...
            Log.d(
                "XClientRequestHandler",
                "X_UngrabServer request handled successfully:" + outputStream.buffer.position());
          }
          break;
        default:
          if (opcode < 0) {
