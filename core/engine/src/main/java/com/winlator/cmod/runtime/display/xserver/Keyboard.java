...
public class Keyboard {
  ...
  public boolean onKeyEvent(KeyEvent event) {
    ...
    // Patches: Crash on Asus Rog Phone 9 Pro with Game Genie active (This line should be deleted
    // after the merge)
    ...
    if (keyCode == KeyEvent.KEYCODE_TAB || keyCode == KeyEvent.KEYCODE_ESCAPE) {
      ...
      if (action == KeyEvent.ACTION_DOWN) {
        ...
        xServer.injectKeyPress(
            xKeycode, xKeycode != XKeycode.KEY_ENTER ? event.getUnicodeChar() : 0);
      } else if (action == KeyEvent.ACTION_UP) {
  ...
  public static boolean isModifier(byte keycode) {
    return keycode == XKeycode.KEY_SHIFT_L.id
