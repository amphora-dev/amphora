...
JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_createEpollFd(
...
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_doEpollIndefinitely(
    JNIEnv *env, jobject obj, jint epollFd, jint serverFd,
  ...
  for (int i = 0; i < numFds; i++) {
    if (events[i].data.fd == serverFd) {
      ...
      (*env)->CallVoidMethod(env, obj, handleExistingConnection,
                             events[i].data.fd);
    }
  }
...
JNIEXPORT jboolean JNICALL
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_addFdToEpoll(
    JNIEnv *env, jobject obj, jint epollFd, jint fd) {
  struct epoll_event event;
  event.data.fd = fd;
  event.events = EPOLLIN;
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_removeFdFromEpoll(
    JNIEnv *env, jobject obj, jint epollFd, jint fd) {
  epoll_ctl(epollFd, EPOLL_CTL_DEL, fd, NULL);
}

JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_ClientSocket_read(
...
JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_ClientSocket_write(
...
JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_createEventFd(
    JNIEnv *env, jobject obj) {
  return eventfd(0, EFD_NONBLOCK);
}

JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_ClientSocket_recvAncillaryMsg(
    JNIEnv *env, jobject obj, jint clientFd, jobject data, jint offset,
    jint length) {
  char *dataAddr = (*env)->GetDirectBufferAddress(env, data);
  ...
  struct msghdr msg = {.msg_name = NULL,
                       ...
                       .msg_control = &ctrlmsg,
                       .msg_controllen =
                           sizeof(struct cmsghdr) + MAX_FDS * sizeof(int)};
...
JNIEXPORT jint JNICALL
Java_com_winlator_cmod_runtime_display_connector_ClientSocket_sendAncillaryMsg(
    JNIEnv *env, jobject obj, jint clientFd, jobject data, jint length,
    jint ancillaryFd) {
  char *dataAddr = (*env)->GetDirectBufferAddress(env, data);
...
Java_com_winlator_cmod_runtime_display_connector_XConnectorEpoll_pollEpollEvents(
    JNIEnv *env, jobject obj, jint epollFd, jint maxEvents) {
