...
/**
 * Kernel-dependent half of [BundledContentSource]: given a SHA-verified archive
 * file (already staged by [BundledContentSource]), extract/install it to the
 * canonical content location and return the resolved path.
 *
 * **Why this is an abstraction in `:core:content` (not a concrete class):** the
 * real impl (`WinlatorBundledAssetInstaller` in `:core:engine`) needs the ported
 ...
 */
interface BundledAssetInstaller {
    /**
